...
def normalize_symbol_for_cid(symbol: str, max_len: int | None = None) -> str:
    """Normalize symbol into a CID-safe token (A-Z0-9 only).

    Binance rejects non-ASCII in newClientOrderId (-1100). For ASCII symbols
    already matching the CID pattern, we keep them unchanged. For anything else,
    we use a stable hash token.
    """
    if not symbol:
        token = "X"
        ...
        digest = hashlib.sha1(symbol.encode("utf-8")).hexdigest().upper()
        token = f"X{digest}"
...
def cid_symbol_matches(symbol: str, parsed_symbol_token: str) -> bool:
    """Check if a real symbol matches the symbol token stored in a CID.

    For ASCII symbols (matching [A-Z0-9]+): the CID stores the full symbol
    with no truncation, so require exact equality.  Using startswith here
    would cause a false positive: "BTCUSDT".startswith("BTC") is True, but
    a CID for the "BTC" symbol must NOT match the "BTCUSDT" whitelist entry.

    For non-ASCII symbols: the CID stores a truncated SHA1 hash token
    (generated by normalize_symbol_for_cid), so check that the full hash
    starts with the stored prefix.
    """
    full_token = normalize_symbol_for_cid(symbol)
    if re.fullmatch(r"[A-Z0-9]+", symbol):
        return full_token == parsed_symbol_token
    return full_token.startswith(parsed_symbol_token)
...
def generate_client_order_id(
    ...
    ts_sec = ts // 1000
    base = f"{config.prefix}{config.strategy_id}_"
    suffix = f"_{level_id}_{ts_sec}_{seq}"
    max_symbol_len = BINANCE_MAX_CLIENT_ORDER_ID_LEN - (len(base) + len(suffix))
    if max_symbol_len < 1:
        raise ValueError(
            f"clientOrderId too long ({BINANCE_MAX_CLIENT_ORDER_ID_LEN} max) "
            f"for prefix/level/ts/seq: {base}{suffix}"
        )
    # For ASCII symbols: truncation loses identity — raise so caller can retry with shorter prefix.
    # For non-ASCII symbols (hash tokens): truncation is expected and allowed.
    full_token = normalize_symbol_for_cid(symbol)
    if len(full_token) > max_symbol_len and re.fullmatch(r"[A-Z0-9]+", symbol):
        raise ValueError(
            f"clientOrderId too long ({BINANCE_MAX_CLIENT_ORDER_ID_LEN} max): "
            f"symbol '{symbol}' ({len(full_token)} chars) exceeds available {max_symbol_len} chars "
            f"in '{base}...{suffix}'"
        )
    safe_symbol = normalize_symbol_for_cid(symbol, max_symbol_len)
    cid = f"{base}{safe_symbol}{suffix}"
