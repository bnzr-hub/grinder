...
- **Flat preserve mode (post-PR6):** `GridV2Config.reseed_on_flat` (env `GRINDER_GRID_V2_RESEED_ON_FLAT`, default `true`) controls full-unwind normalization. `true` keeps legacy auto-reseed behavior. `false` disables full recenter (`RECENTER*`) but keeps rolling continuity (`EXIT_RESTORE`) so missing side levels are replenished on unwind; FLAT integrity still performs cleanup cancels for extra entry/exit drift (`GRID_V2_INTEGRITY_FLAT_PRESERVE`).
