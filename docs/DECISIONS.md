...
- **Status:** Implemented as feature-flagged architecture (`GRINDER_GRID_V2_ENABLED` / `GRINDER_GRID_V2_SHADOW`, default off), with post-PR6 hardening continuing. Existing doc-26 code remains operational.
...
- **Context / why replace:** The doc-26 line grew through 11+ sub-versions (V1A-V1G), accumulating complexity that is hard to reason about, verify with live proof, and extend. Key problems: (1) no entry/exit ownership separation — grid orders and TP/exit orders share planner space, causing ownership conflicts (TP slot exclusion, `diff_extra_tp`, cross-tick overlap); (2) no explicit inventory model — fill detection relies on heuristics (disappearance, inflight CID tracking), not a proper lot ledger; (3) unbounded outward growth managed via bolted-on guards (freeze, anti-churn, convergence); (4) recenter while exposed — continuous mid-price recentering loses position relative to open fills.
- **New model core:** Explicit state machine with separation of concerns. Entry window (bounded, rolling), inventory ledger (lot-level tracking), exit map (paired exits outside window), rolling controller (entry-only mutations with opposite-edge trim). Deterministic transitions, exchange-truth reconciliation, one-sided inventory mode.
- **Architectural choices (v1, binding):** (1) one-sided inventory + two-sided rolling entry window (opposite-edge trim) = YES; (2) flat-only recenter = YES; (3) mixed inventory forbidden = YES; (4) exits outside rolling window = YES; (5) flat normalization = keep rolled window, optional explicit recenter.
- **Migration plan:** PR1 (spec only, no code) → PR2 (pure state machine + 20 acceptance tests) → PR3 (reconciliation adapter) → PR4 (execution integration) → PR5 (paper/shadow verification) → PR6 (small live ceremony). **Status: completed through PR6.**
...
- **PR3 delivered:** Reconciliation adapter (`src/grinder/grid_v2/adapter.py`, 818 lines). 99 tests across 16 classes. Section 22 (22.1-22.11) added to doc-27 spec. CID scheme (strategy `g`), bidirectional order registry with reverse-key collision guards, fill translation, action resolution, reconciliation detection, snapshot reconstruction (8 fail-closed rules), symbol-scoped CID ownership. Core invariant: translation never mutates registry. No exchange I/O, no dispatch. Comment-only touch to `reconcile/identity.py` (strategy reservation).
- **PR4 delivered:** Runtime bridge (`src/grinder/grid_v2/bridge.py`) + engine wiring (`src/grinder/live/engine.py`). 33 tests across 19 classes. Section 23 (23.1-23.9) added to doc-27 spec. `GridV2Bridge` wires adapter + state machine into `ExecutionAction` pipeline. Binding runtime contract (strict call order: translate → apply → confirm → resolve → dispatch). Startup fail-closed (reconstruction must succeed before any dispatch). Fill/cancel lifecycle with full observability (`FillResult`, `CancelAckResult`). Engine integration: `GRINDER_GRID_V2_ENABLED`/`GRINDER_GRID_V2_SYMBOL` env vars (fail-closed on empty symbol), bridge construction at init, startup on first account sync, fill detection via registry-vs-exchange diff, cancel-ack routing via pending-cancel tracking, seed action dispatch on startup tick, non-flat position fail-closed guard, grid_v2 action generation branch in `process_snapshot`.
- **PR5 delivered:** Shadow verification (`src/grinder/grid_v2/shadow.py`). 26 tests across 13 classes. Section 24 (24.1-24.5) added to doc-27 spec. `GridV2ShadowRunner` runs grid_v2 in parallel on same snapshots via isolated bridge instance. `GRINDER_GRID_V2_SHADOW` env var (mutually exclusive with primary `GRINDER_GRID_V2_ENABLED`). Shadow never injects actions into dispatch pipeline. Fail-open error handling. Divergence detection: count/type mismatch between shadow and legacy actions. Smoke proof script `scripts/smoke_grid_v2_shadow.sh`.
- **PR6 delivered:** Live ceremony PASSED (2026-03-18). Price quantization: side-aware `_quantize_price(price, side)` (BUY=ROUND_DOWN, SELL=ROUND_UP) + `GRINDER_GRID_V2_TICK_SIZE` required env var (fail-closed). Awaiting-sync gate: `_grid_v2_awaiting_sync` + `_grid_v2_pending_seed_cids` — skips fill detection until ALL seed CIDs visible in account snapshot. 9 new tests (42 bridge tests, 23 classes). Ceremony: BTCUSDT, 1 level, 0.002 BTC, 180s, 16167 ticks, 0 errors, FLAT position, cleanup CLEAN. Section 25 (25.1-25.3) added to doc-27 spec.
- **Market-data venue fix (post-PR6):** `--exchange-port futures` switches WS market data to `fstream.binance.com` (was `stream.binance.com` spot). Wired end-to-end via `BinanceWsConfig.ws_url_override`. Symbol-vs-venue preflight: fail-closed if constraints unavailable or symbol not in futures exchangeInfo. Venue proof: FARTCOINUSDT 52 msgs on fstream, 0 on spot stream.
- **Pending-place visibility gate (post-PR6):** `_grid_v2_pending_place_cids: dict[str, int]` tracks EXECUTED PLACE CIDs until visible in account snapshot or 2-sync-cycle grace expires. Prevents false-fill cascade on newly placed CIDs. Lifecycle: BLOCKED/SKIPPED → clean (never sent). FAILED + `pre_send=True` (local validation, CircuitOpenError) → clean. FAILED + definitive `exchange_code` (e.g. -1111/-4014/-4164) → clean. FAILED + ambiguous `exchange_code` (-2010 only, duplicate CID after retry) or no code (MAX_RETRIES, network error) → quarantine as pending, snapshot resolves. `-2019` (margin insufficient) is a definitive reject, not ambiguous. `_AMBIGUOUS_EXCHANGE_CODES` set governs classification. `ConnectorNonRetryableError.pre_send` + `.exchange_code` classify error provenance. 70 bridge tests (31 classes).
- **Post-PR6 fill-processing hardening:** Engine disappeared-fill order is deterministic `EXIT -> ENTRY` (same-tick unwind/branch-switch safety). If bridge returns rejected for a disappeared CID, engine immediately cleans that CID from registry (`confirm_cancel_*`) and logs `GRID_V2_REJECTED_FILL_CLEANED`, preventing repeat reject loops.
- **Flat preserve mode (post-PR6):** `GridV2Config.reseed_on_flat` (env `GRINDER_GRID_V2_RESEED_ON_FLAT`, default `false`) controls full-unwind normalization. `true` enables unconditional full recenter (`RECENTER*`). `false` keeps rolling continuity (`EXIT_RESTORE`) so missing side levels are replenished on unwind; FLAT integrity still performs cleanup cancels for extra entry/exit drift (`GRID_V2_INTEGRITY_FLAT_PRESERVE`). By default, preserve mode uses skew-only reseed via `GRINDER_GRID_V2_RESEED_ON_FLAT_ONLY_ON_SKEW=true`: in preserve mode, recenter is re-enabled only when FLAT ladder is BUY/SELL skewed.
- **User-data unknown event observability hardening (post-PR6):** unknown user-data WS events now emit bounded per-type counters in connector metrics (`grinder_user_data_unknown_events_total{event_type=...}` with `known` + `other` bucketing) and connector-local breakdown in `UserDataWsStats`. Logging remains fail-open; no changes to dispatch/trading decisions.
- **Grid_v2 warning observability + alert thresholds (post-PR427):** added live-engine counters for transient mismatch and rejected-fill cleanup paths: `grinder_live_grid_v2_integrity_mismatch_pending_total{sym}` and `grinder_live_grid_v2_rejected_fill_cleaned_total{sym,source,reason}`. Added warning alerts with short-horizon thresholds: `GridV2IntegrityMismatchPendingBurst` (>5/15m) and `GridV2RejectedFillCleanedBurst` (>3/15m). Unknown user-data warning logs now include explicit `event_type` in log message for triage speed.
- **Launch guard v2 + crash-safe recovery:** `run_trading.py` startup for futures+mainnet+armed runs `preflight → recovery policy → verify → start` (fail-closed). Recovery policy: (1) non-flat position → cleanup NEVER executed (would close position), startup proceeds for grid_v2 reconstruction; (2) FLAT + dirty → requires 2 stable snapshots before cleanup (guards against pending fills arriving between reads). New statuses: `recovery_non_flat_skip_cleanup`, `recovery_snapshot_unstable`. Pre-cleanup and cleanup-on-exit default ON. `exchange_state.py` exposes `cmd_verify_programmatic()` for programmatic verification.
- **F2 protective recovery (startup):** When restart finds non-flat position but no exit orders on exchange (F2), adapter synthesizes protective lot + exit from position_qty/reference_price instead of raising ValueError. Bridge generates real CIDs and PLACE_EXIT actions (reduce-only). Engine emits protective exits and blocks entries via awaiting_sync until exits confirmed. Covers both cases: grid_v2 entries exist but no exits, and no grid_v2 orders at all. Uses `PositionSnap.signed_qty` (original `positionAmt` from Binance) for correct exit side: LONG→SELL reduce-only, SHORT→BUY reduce-only. Definitive PLACE failures (BLOCKED/SKIPPED/non-ambiguous exchange reject) clear `pending_seed_cids` to prevent awaiting_sync deadlock.
- **Grid_v2 startup recovery modes:** `GRINDER_GRID_V2_RECOVERY_MODE` env (default `restore_then_cleanup_reseed`). On reconstruction failure: `restore_then_cleanup_reseed` runs exchange cleanup → verify CLEAN → startup_fresh; `restore_then_block` stays fail-closed. Cleanup fallback guarded by `ALLOW_MAINNET_TRADE=1`. If cleanup or verify fails → fail-closed (no dispatch).
- **Net-off transition (Variant B, PR-A):** `GRINDER_GRID_V2_NETOFF_ENABLED` env (default off). Architectural decision: opposite-side entry fill in branch mode closes closest lot (net-off) instead of `BRANCH_INCOMPATIBLE` reject. Enables dual-sided rolling ladder without mixed inventory. Paired exit canceled (not filled). SM contract change only; engine repair changes in PR-B.
- **Integrity repair anti-churn guards (PR-B):** Three guards prevent repair churn: (1) pending-slot dedup (`(side, price)` in `pending_place_cids` → skip); (2) distance guard (`GRINDER_GRID_V2_REPAIR_MAX_DISTANCE_STEPS`, default 5 — skip entries >N steps from mid); (3) budget cap (`GRINDER_GRID_V2_REPAIR_MAX_ACTIONS_PER_CYCLE`, default 5 — max PLACEs per repair cycle). Guards apply only to integrity repair path, not seed/F2.
- **Shadow selector Phase 1 (doc-36):** `ShadowSelector` wired into `LiveEngineV0.process_snapshot()` post-dispatch. Reuses `select_topk_v1` scoring. Env-gated `GRINDER_SYMBOL_SELECTOR_SHADOW=1` (default off). Timestamp-based cooldown (`CYCLE_S`). Fail-open: selector exceptions logged, engine continues. Metrics: 5 families (`grinder_selector_{cycle,candidate_count,excluded,churn,score_bps}_total`). Score cardinality capped to top-K + 2. Hypothetical churn only (would_add/would_remove). No dispatch-universe mutation.
- **Active selector Phase 2 (doc-36):** `ActiveSelector` wraps `ShadowSelector` scoring + adds hysteresis (`MIN_HOLD_CYCLES`, `ENTER/EXIT_THRESHOLD_BPS`), bounded transitions (`MAX_CHANGES_PER_CYCLE`), graceful_exit_only for grid_v2 non-flat symbols. Env-gated `GRINDER_SYMBOL_SELECTOR_ENABLED=1` (default off). Operator-universe only (no auto-scan). Fail-safe: on runtime error, retains previous stable active set for that cycle. 3 additional metric families (`active_set_size`, `transition_total`, `graceful_exit_only_gauge`). Initial active set = operator symbols from CLI.
- **ML-assisted scoring Phase 3 (doc-36):** Bounded ML adjustment in `ShadowSelector._apply_ml_adjust()`. Env: `GRINDER_SYMBOL_SELECTOR_ML_ENABLED=1` + `ML_ADJUST_MAX_BPS` (cap). `ml_adjust_provider` callback returns per-symbol adjust_bps, clamped to [-cap, +cap], applied after baseline topk_v1 scoring. Re-rank with deterministic tie-break. Fail-open: provider error/empty → fallback to baseline, no crash. Shadow only — no dispatch mutation. 3 ML metric families (`ml_adjust_applied_total`, `ml_fallback_total`, `ml_adjust_bps{rank}`). Provider-missing observability: explicit `SELECTOR_ML_PROVIDER_MISSING` warning + startup hint (no behavior change).
- **Active ML Phase 3b (doc-36):** ML adjust wired into `ActiveSelector` via `ShadowSelector` delegation. ONNX provider (`src/grinder/selection/ml_provider.py`) wired in `build_engine` via `GRINDER_ML_REGIME_MODEL_DIR`. Regime → adjust: HIGH=+base, MID=0, LOW=-base (base = max_bps/2). `_feature_cache` shared with provider closure for zero-copy feature access. Fail-open: model load error or predict failure → baseline fallback. All Phase 2 safety invariants preserved.
- **Symbol Risk Manager v1:** Per-symbol risk escalation layer (Gate 10 in `_process_action`). States: NORMAL → CAPPED → EXIT_ONLY. CAPPED/EXIT_ONLY block INCREASE_RISK for the symbol; reduce-risk/cancel always pass. Only safety-critical grid_v2 actions bypass Gate 10 (cancel, reduce-only, integrity cancel). Escalation triggers: notional exceeds `MAX_NOTIONAL_USD`, qty exceeds `MAX_POSITION_QTY`, consecutive losses ≥ `MAX_CONSECUTIVE_LOSSES` (per closed lot P&L, goes directly to EXIT_ONLY). De-escalation: step down one level when conditions safe + cooldown expired + EXIT_ONLY TTL passed. Feature-flagged: `GRINDER_SYMBOL_RISK_ENABLED=0` default. Applies to grid_v2 symbols only by default (`APPLIES_TO_GRID_V2_ONLY=1`).
- **Symbol Unload Controller v1:** Staged reduce-only position unload in EXIT_ONLY state. Activated automatically by risk manager escalation. Produces single reduce-only PLACE per step cycle. LONG→SELL, SHORT→BUY. Step qty = max(abs_pos × step_pct, min_step_qty). Bounded by cooldown (`STEP_COOLDOWN_S`), hourly cap (`MAX_STEPS_PER_HOUR`), reprice TTL + max reprices per step. Flat → UNLOAD_COMPLETE. 3 consecutive failures → PAUSED (fail-closed, stays EXIT_ONLY). Feature-flagged: `GRINDER_SYMBOL_UNLOAD_ENABLED=0` default.
- **Adaptive Step Controller v1:** Volatility-aware grid spacing. Effective step = `base_pct × (natr_bps / vol_ref_bps)`, clamped to `[STEP_MIN_PCT, STEP_MAX_PCT]`. Hysteresis (`HYSTERESIS_BPS`) prevents oscillation. Cooldown (`UPDATE_COOLDOWN_S`) prevents churn. Fail-open: `STEP_FAIL_MODE=freeze_last` (keep last known) or `fallback_base` (revert to base). v1: applies to bridge recreation (reseed/restart) only, no mid-flight SM config mutation. Feature-flagged: `GRINDER_GRID_V2_ADAPTIVE_STEP_ENABLED=0` default.
- **Geometry watchdog repair (role-aware):** Integrity repair separated by order role. ENTRY: tick-level geometry matching via `match_entries_with_tolerance()`, fuzzy tolerance, anti-churn guards. EXIT: lot-ledger integrity check — expected exit CIDs from SM `exit_orders` vs exchange. ENTRY cancel only targets ENTRY CIDs; EXIT cleanup only targets EXIT CIDs. Missing exits in branch mode logged for observability. Same-tick fill/repair dedup. Strict geometry mode for branch repair distance bypass. Exit orphan accounting is `OPEN`-only (historical `FILLED/CANCELED` exits excluded), delivered in PR #449 and validated by post-merge live smoke (`CLEAN`, no repeated `registry_orphans` mismatch).
...
- **Stale model:** Dual TTL: soft stale (`GRINDER_RISK_BASE_STALE_TTL_S`, default 30s) warns and will block INCREASE_RISK in PR-2. Hard stale (`GRINDER_RISK_BASE_MAX_AGE_HARD_S`, default 60s) is fail-closed marker for PR-2.
- **API freeze (PR-1):** `RiskBaseSnapshot` fields: `value_usd`, `mode`, `asof_ts_ms`, `age_s`, `is_stale_soft`, `is_stale_hard`, `source_fields_present`, `is_below_min`.
...
### ADR-093: Geometry Watchdog Repair Hardening (2026-03-26)

- **Status:** Delivered.
- **Decision:** Enable real geometry mismatch detection and fix convergence/streak bugs that prevented repair from triggering.
...
- **Helper:** `_occupied_prices()` method returns union of open exit order prices + open lot entry prices + all current entry window prices.

### ADR-098: Exit Price Dedup + Distance Clamp (2026-03-26)
...
### ADR-129: Deactivation finalize gate (2026-04-01)

- **Decision:** Create `src/grinder/orchestration/deactivation.py` — pure finalize gate for symbol removal. Answers: "given current runtime facts, is it safe to finalize?"
- **Phase:** C2a. Pure logic, no live cleanup execution, no engine commands, no exchange I/O.
- **Contract:** `can_finalize_deactivation(symbol, facts) -> DeactivationDecision` with `can_finalize` and `block_reason`.
...
### ADR-136: Graceful exit signal and engine deactivation ceremony (2026-04-01)
...
- **Decision:** Add fixture-natural integration tests T63-T65. Zero internal state mutation — only external signals (account snapshots, sync gen advancement). Engine manages `_prev_rolling_orders`, `_inflight_shift`, and pending cancels entirely on its own through `process_snapshot()`. T63: init → orders confirmed → exchange empties + flat → ANCHOR_RESET → new grid from new anchor. T64: engine cleanup ran (offset reset). T65: deterministic.
...
### ADR-109 Phase 3 PR-1: Shadow PositionLedger from ACCOUNT_UPDATE (2026-04-05)

- **Problem:** Position truth is snapshot-only (REST positionRisk). No event-driven position model exists. No visibility into whether ACCOUNT_UPDATE position events agree with REST snapshots.
...
### ADR-163: Preserve controller-owned symbols in execution desired set (2026-04-08)

- **Problem:** In the first multi-symbol live canary, 4USDT and BULLAUSDT activated and traded normally. On the next ranking cycle, the admitted set shifted to different symbols. The reconciler flagged the live engines as ORPHAN (present but not in desired), STL_E3_ORPHAN_ENGINE fired, and the coordinator was blocked from all execution actions — including the graceful-exit that would have resolved the situation. Engines kept trading, but orchestration froze.
- **Root cause:** `_run_execution_phase` built `desired = set(decision.admitted[:top_k])` — purely from the current ranking cycle. It did not include symbols the rotation controller already owned as ACTIVE or GRACEFUL_EXIT_ONLY.
...
- **2026-04-11 smart TTL refinement:** The original age-only rule produced ~1 sync-cycle coverage gaps on stable symbols (live-observed on AKEUSDT post-#668 verification canary) because healthy in-set entries were cancelled unconditionally and re-placed at the same slot. Refinement: before appending an aged CID to `_stale_cancels`, check whether its `(side, price)` is still present in the current `effective_entry_keys` computed by the same pipeline `reconcile_grid_state` uses. If yes — skip. The check is gated by "there is at least one aged candidate" so the effective-keys computation is lazy and has no cost when no entries are aged. The original MAGMA/RAVE protection is preserved: orphans, post-transition stale topology, and true far-from-grid seeds are still retired. `compute_effective_entry_keys` lives in `grid_v2/sync_reconciler.py` so both callers never disagree about what "still on-grid" means. `GRID_V2_STALE_ENTRIES_RETIRED` log gains a `skipped_in_set=N` field for operator visibility; the event is also emitted with `count=0` when only skips occurred.
- **2026-04-11 skip-only log throttle:** The post-#669 verification canary (30 min, mainnet, 3 active symbols) emitted 163 `count=0 skipped_in_set=N` log lines on stable symbols (GWEI 127, MAGMA 28, ARIA 8). Predicted P2 from the #669 review materialized: skip-only steady state was logging on every sync cycle after the first TTL expiry. Refinement: the skip-only path now uses log-on-change semantics with a periodic keepalive of `_GRID_V2_STALE_SKIP_LOG_KEEPALIVE_GENS = 60` cycles (~5 min at 5 s sync cadence). State is per-engine (per-symbol) via two scalars `_grid_v2_stale_skip_last_count` (sentinel `-1` = "never logged") and `_grid_v2_stale_skip_last_log_gen`. Real retire (`count > 0`) is **unthrottled** and refreshes the skip throttle state so the next skip-only event after a cancel batch is treated as new context. No behavior change to the smart-TTL decision logic. Operator visibility preserved: change-driven log + keepalive heartbeat is sufficient to confirm the skip path is alive on quiet symbols.

### ADR-182: FLAT re-symmetrize when reseed cooldown suppresses flat reseed (2026-04-11)

- **Problem:** Investigation round 1 on `state.py` (post-#670, sparked by an ARIAUSDT live observation) found a corner case where the symbol could end up in `mode=FLAT` with a one-sided branch-shaped entry window indefinitely. The repro: branch entry zeros the opposite side via `ONE_SIDED_CANCEL_OPPOSITE`; first full unwind back to FLAT triggers the genuine flat reseed (`RECENTER`), bumping `last_recenter_ts`; if a second branch entry happens shortly after and unwinds again within `reseed_cooldown_ms = 30_000` (default), the cooldown gate sets `_flat_reseed = False, _reseed_suppressed = "cooldown"`. The legacy fall-through then enters `if new_open or restore_without_reseed:` which is the **in-branch one-sided EXIT_RESTORE** path. That path explicitly does not restore the opposite side (intentional in-branch behavior). End state: `mode=FLAT` with one side empty, stable until the next branch entry. Operationally observed as "сетка после exit chain не перестроилась".
- **Decision:** Add a new branch in `_apply_exit_filled` between the legacy fall-through and the genuine reseed. When `_returning_to_flat AND _reseed_suppressed == "cooldown"`, build a fresh symmetric entry window via `_build_entry_window(snap.entry_window.reference_price, ...)`, diff against the old branch-shaped window, and emit only the actual deltas: `CANCEL_ENTRY` for prices in the old window that are not in the new symmetric one (reason `FLAT_RESYMMETRIZE_REPLACE`) and `PLACE_ENTRY` for prices in the new symmetric window that are not in the old one (reason `FLAT_RESYMMETRIZE`). **Crucially, `last_recenter_ts` is NOT advanced** — the cooldown stays armed against the original genuine recenter so the gate remains correct for future events. This is a "re-symmetrize", not a "recenter".
- **Invariant:** `mode == FLAT` implies the entry window is symmetric (both sides populated within `entry_levels_per_side`). The intentional in-branch one-sided rule (`LONG_BRANCH` zeros sells, `SHORT_BRANCH` zeros buys) is unchanged. The legacy operator-opt-in preserve path (`reseed_on_flat=False AND reseed_on_flat_only_on_skew=False`, atypical config) is also unchanged: that case routes through `restore_without_reseed AND not _resymmetrize_flat`.
- **Constraints preserved:** `reseed_cooldown_ms` semantics intact (no removal, no weakening). In-branch `EXIT_RESTORE` unchanged. First branch entry behavior (`ONE_SIDED_CANCEL_OPPOSITE`) unchanged. No reconciler/bridge changes — fix is fully scoped to `state.py::_apply_exit_filled`. ADR-177/178 burst suppression and ADR-179 stale aging are not touched.
- **Consequences:** FLAT can no longer persist with a one-sided window after a fast double-unwind. The next position cycle on the symbol begins from a usable symmetric grid regardless of how fast the previous unwind happened. Actions emitted under the new path use distinct reasons (`FLAT_RESYMMETRIZE` / `FLAT_RESYMMETRIZE_REPLACE`) so the path is identifiable in execution logs and proof bundles. Tests in `tests/unit/test_grid_v2_state.py::TestH4FlatResymmetrize` cover: (1) the exact double-unwind-within-cooldown scenario, (2) `last_recenter_ts` not bumped, (3) cooldown still arms against subsequent genuine recenter, (4) first unwind without prior recenter still uses `RECENTER`, (5) in-branch `EXIT_RESTORE` unchanged, (6) diff is minimal not full replacement, (7) `reseed_cooldown_ms=0` config bypasses the new path entirely.

### ADR-183: Dynamic bootstrap candidate discovery — PR-1 observational scan (2026-04-11)

- **Problem:** The autonomous runtime performs universe discovery, prefiltering, ranking, and admission on every cycle, but the **tuning candidate set** is effectively frozen to whatever passed bootstrap tuning at process startup. `AutonomousTuningState.candidates` is populated in `__init__` and never mutated; `TuningRefresher._run_one_cycle` refreshes only those existing symbols. A symbol that later becomes prefilter-eligible (price recovers, volume spikes, balance grows such that min-notional passes, etc.) cannot enter the tuning pipeline without restarting the process. Post-#671 canary on 2026-04-11 exhibited this: initial balance allowed only SIRENUSDT to pass cold bootstrap notional gates, so the system stayed single-symbol for the entire 30-minute run even though conditions for other symbols likely changed.
- **Decision (PR-1, observational):** Add bounded periodic dynamic candidate discovery to `TuningRefresher`. On each refresh cycle, if a `universe_provider` + coarse-select + prefilter callables are injected, the refresher calls the same two-stage filter the cold bootstrap uses: top-N by 24h quote volume → volume/NATR/spacing/blacklist prefilter. The result is subtracted against the current `AutonomousTuningState.candidates` and the remaining "new" subset is bounded by `dynamic_bootstrap_max_new_per_cycle = 5`. The selected list is **only logged and stored on `self._last_dynamic_candidates`**; it is NOT tuned and NOT merged into the shared state. Observability events: `DYNAMIC_BOOTSTRAP_DISCOVERY_START`, `DYNAMIC_BOOTSTRAP_DISCOVERY_RESULT discovered=X coarse=Y prefiltered=Z new=W selected=K`, `DYNAMIC_BOOTSTRAP_DISCOVERY_SELECTED symbols=...`. Failure modes (`DYNAMIC_BOOTSTRAP_UNIVERSE_FETCH_FAILED`, `DYNAMIC_BOOTSTRAP_COARSE_SELECT_FAILED`, `DYNAMIC_BOOTSTRAP_PREFILTER_FAILED`, `DYNAMIC_BOOTSTRAP_DISCOVERY_FAILED`) are all fail-open: the refresh path continues unaffected.
- **Invariant (PR-1):** No mutation to `AutonomousTuningState` from the discovery path. `state.candidates` remains whatever `build_runtime` populated from cold bootstrap. Active engines are never affected. Cold bootstrap behavior (`scripts/run_autonomous.py::_bootstrap_tuning_cache` and the private `_select_bootstrap_subset`/`_apply_bootstrap_prefilter` helpers) is untouched — the refresher reuses them via dependency injection from `_build_tuning_state_and_selector`, which preserves existing test patches on `run_autonomous.mod._apply_bootstrap_prefilter`.
- **Scope limits:** This is PR-1 only. **Not in this PR:** atomic merge of newly tuned symbols into `AutonomousTuningState` (PR-2), retry/backoff for repeatedly-failing candidates (PR-3), operator-facing `--top-k` semantic clarification (optional PR-4), changes to cold bootstrap flow, changes to admission gate, changes to rotation controller, hot-swap of active engine config.
- **Consequences:** The refresher now emits a steady cadence (every `_DEFAULT_REFRESH_INTERVAL_S = 240 s`, 4 min) of observational discovery events. Operators can watch `DYNAMIC_BOOTSTRAP_DISCOVERY_RESULT` to confirm the system would be able to pick up new symbols if PR-2 existed, and they can see which specific symbols would be selected. No behavior change for any active runtime path. Backwards compatibility is preserved: `TuningRefresher` instantiated without the discovery plumbing args (legacy call sites, tests) is a no-op on the discovery branch and behaves identically to the pre-ADR-183 refresher. Test coverage in `tests/unit/test_tuning_refresher_discovery.py` includes: discovery disabled when plumbing missing, new-vs-known distinction, bounded selection, fail-open for all three external dependencies (universe / coarse / prefilter), state isolation (candidates and tuned_results unchanged), blacklist propagation, observability event structure, backwards compat with legacy constructor, and default constant alignment with cold bootstrap tunables.

- **2026-04-11 PR-2 dynamic tuning admission:** Completes the dynamic bootstrap path by actually tuning the PR-1 discovered subset and merging successfully tuned symbols into shared state under a single atomic `replace()`. New method `TuningRefresher._tune_dynamic_candidates(candidates, symbol_risk_budget, mainnet)` runs the full bootstrap-equivalent pipeline (features → NATR → retune → v2 features) on the bounded dynamic subset, filters every map-shaped output to the successful-only subset, and returns the result alongside `all_results` for cache bookkeeping. `_run_one_cycle` is restructured to: (1) refresh existing tuned candidates via the legacy path unchanged, (2) if `_last_dynamic_candidates` is non-empty, call `_tune_dynamic_candidates` in an isolated try/except (fail-open — a dynamic tuning crash does not break existing refresh commit), (3) merge existing and newly tuned maps into a single snapshot, (4) extend `candidates` by exactly the successfully tuned new symbols in discovery rank order, (5) commit cache entries for every attempted symbol (existing + new), (6) call `state.replace()` **once** with the merged snapshot including the extended `candidates` list, (7) update bridge future-activation config for the merged tuned set. `AutonomousTuningState.replace()` gains an optional `candidates: list[str] | None` parameter (default `None` preserves legacy behavior where candidates is never replaced) — supplying `candidates` atomically swaps it inside the same lock that guards the other five map fields.
- **Atomic visibility invariant (PR-2):** For every symbol in `state.candidates`, that symbol is also in `tuned_results`, `tuned_sizes`, `natr_map`, `v1_features`, and `v2_features`. Conversely, no symbol appears in any map field without being in `candidates`. Failed dynamic tunings are never merged — they land only in `all_results` for cache bookkeeping, never in any shared-state map. The invariant holds by construction because `_tune_dynamic_candidates` filters all map-shaped outputs to the successful subset before returning, and `_run_one_cycle` extends `candidates` with exactly that same successful subset. The invariant is structurally enforced at the commit boundary, so concurrent selector/admission readers never observe a torn snapshot.
- **Scope preserved (PR-2 does NOT include):** retry/backoff policy for repeatedly-failing dynamic candidates (PR-3), operator-facing `--top-k` semantic clarification (optional PR-4), hot-swap of active engine geometry for re-tuned symbols (never), selector/admission pipeline bypass (never), cold bootstrap flow changes.
- **Consequences (PR-2):** A symbol absent from the initial cold bootstrap tuned set can now enter shared state via the refresher cadence (~4 min) and become eligible for selector ranking, risk admission, and activation without restart. This closes the functional gap behind issue #663 at the tuning-state seam while leaving the rotation controller, admission gate, and live engine factory untouched. `DYNAMIC_BOOTSTRAP_STATE_EXTENDED admitted=N total_candidates=M version=V` logs the successful admission events so operators can observe the transition live. Test coverage in `tests/unit/test_tuning_refresher_dynamic_admission.py` (17 tests, 8 classes) includes: fresh admission from empty initial state, merge over existing candidates, mixed-result tuning where only successful symbols merge, all-dynamic-fail leaves state unchanged, discovery rank order preserved, empty dynamic is backwards-compatible with legacy refresh, `_tune_dynamic_candidates` direct unit tests (successful, mixed failures, empty input), dynamic-tuning exception fail-open preserves existing refresh commit, single `state.replace()` call per cycle, `state.version` advances after commit. The load-bearing test `_assert_state_coherent` asserts the atomic visibility invariant structurally after each commit.

- **P1 fix (2026-04-11 review):** The initial PR-2 draft still had a torn-snapshot path. `merged_candidates = [*candidates, *newly_admitted]` unconditionally carried forward the existing candidate list, while the existing-refresh path only populated `tuned_results` / `tuned_sizes` / `v2_features` for symbols that succeeded retune this cycle. If an existing candidate failed retune (solver `NO_GO`, missing price, missing constraints, missing NATR) while a dynamic candidate succeeded, commit produced a violation: the failed existing symbol remained in `state.candidates` but was absent from at least three of the five map fields. **Fix:** (1) Before building `merged_candidates`, snapshot the previous committed state and backfill coherent entries for any existing candidate that failed retune this cycle when the previous commit had a fully-coherent entry (all five map fields present). Logged as `TUNING_REFRESH_BACKFILLED count=N symbols=...`. (2) Build `merged_candidates` via strict intersection of all five merged map fields — `sym ∈ merged_candidates` iff `sym` is in all of `merged_tuned_sizes`, `merged_tuned_results`, `merged_natr_map`, `merged_v1_features`, `merged_v2_features`. Preserves discovery rank order (existing first, then newly admitted). (3) Scrub all five merged map fields to exactly the final `merged_candidates` key set so the inverse invariant ("no map key without candidates") also holds structurally — this closes the legacy leak where `fetch_selection_features` populated `v1_features` for every fetched symbol including those that later failed tune. Dropped-without-backfill symbols are logged as `TUNING_REFRESH_DROPPED count=N symbols=...`. Regression coverage in new test class `TestExistingCandidateFailsRetune` (4 tests): existing fails with backfill preserves prior snapshot, existing fails without backfill drops from candidates, mixed existing success+failure+dynamic, and the inverse invariant (no orphan map keys for untuned symbols fetched this cycle).
...
### ADR-181: Resolve fill-consumed entry CIDs out of pending-visibility tracking (2026-04-10)

- **Problem:** Post-ADR-180 live canary (main @ `0bd650f`, 2026-04-10) showed a new P0 entry-tracker pathology on fast-fill small-cap symbols (MAGMAUSDT, AKEUSDT). Both active symbols stuck at `actual_entries=2 missing=3` for 15+ minutes, `GRID_V2_NEVER_SEEN_ENTRY_CLEANED` fired 44 times (cadence accelerating 1/min → 3/min on AKE), `GRID_V2_FILL_REJECTED reason=DUPLICATE_ENTRY_FILL` fired 8 times, `POSITION_LEDGER_SHADOW_DIVERGENCE POSITION_MISSING_IN_SNAPSHOT` fired 333 times on MAGMA in the final 3 minutes. ADR-180's `GRID_V2_STALE_REGISTRY_CLEANED` counter stayed at `0` — this is a different P0 that ADR-180 did not target.
- **Root cause:** On fast-fill symbols, an entry placed at a tight ladder level can be filled between POST and the next `openOrders` snapshot (sync interval ≈ 5 s). The user-data fill path processes the fill correctly (state machine transitions entry → exit, exit is placed), and adds the CID to `_grid_v2_user_fill_seen` for dedup. But it never pops the CID from `_grid_v2_pending_place_cids`, `_grid_v2_fill_eligible_cids`, or `_grid_v2_entry_reg_gen`. After `_GRID_V2_NEVER_SEEN_MAX_GENS` (12) sync cycles with the CID never visible on any snapshot, the ADR-180 bounded never-seen fallback treats it as a dead placement and calls `confirm_cancel_entry` on a CID that was legitimately consumed. A subsequent REST-reconciliation replay of the same fill is then rejected as `DUPLICATE_ENTRY_FILL`, which further corrupts entry tracking via `GRID_V2_REJECTED_FILL_CLEANED`. Cumulatively, the engine loses the ability to map ledger positions to open orders → cascade to `POSITION_LEDGER_SHADOW_DIVERGENCE`.
