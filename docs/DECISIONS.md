...
- **Geometry watchdog repair:** Extends integrity repair with tick-level price alignment checking. Uses `match_entries_with_tolerance()` to fuzzy-match actual orders to expected keys within `EPSILON_TICKS × tick_size`. Orders within tolerance are silently "matched" (prevents false structural churn). Anti-churn: cooldown + per-cycle budget. Enabled by default. Same-tick fill/repair dedup: fill-path PLACE_ENTRY slots are excluded from repair to prevent duplicate orders at same price. Strict geometry mode (`GRINDER_GRID_V2_REPAIR_STRICT_GEOMETRY=0` default): when on, structural missing slots bypass distance guard (budget still applies).
