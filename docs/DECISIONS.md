...
- **Geometry watchdog repair:** Extends integrity repair with tick-level price alignment checking. Uses `match_entries_with_tolerance()` to fuzzy-match actual orders to expected keys within `EPSILON_TICKS × tick_size`. Orders within tolerance are "matched" (prevents false structural cancel/place churn). Orders beyond tolerance trigger geometry cancel. Anti-churn: cooldown + per-cycle budget. Enabled by default.
