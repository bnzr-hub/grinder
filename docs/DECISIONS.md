...
- **Flat preserve mode (post-PR6):** `GridV2Config.reseed_on_flat` (env `GRINDER_GRID_V2_RESEED_ON_FLAT`, default `true`) controls full-unwind normalization. `true` keeps legacy auto-reseed behavior. `false` preserves the current entry window when mode returns to FLAT and disables forced FLAT reseed/placement; integrity still performs cleanup cancels for extra entry/exit drift (`GRID_V2_INTEGRITY_FLAT_PRESERVE`).
