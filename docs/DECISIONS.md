...
- **Doc-36 Phase 0 contract baseline (docs-only):** `docs/36_MULTI_SYMBOL_ELIGIBILITY_ML_INTEGRATION_V1.md` now freezes initial selector contract artifacts before any runtime wiring: env config contract, deterministic exclusion reason codes, and initial selector metrics schema. Decision: Phase 1 implementation remains shadow-only (observability path, no dispatch-universe mutation).
