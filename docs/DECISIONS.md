...
- **Doc-36 Phase 0 contract baseline (docs-only):** `docs/36_MULTI_SYMBOL_ELIGIBILITY_ML_INTEGRATION_V1.md` now freezes initial selector contract artifacts before any runtime wiring: env config contract, deterministic exclusion reason codes, initial selector metrics schema, and configurable threshold contract. Baseline includes `GRINDER_SYMBOL_SELECTOR_MIN_NATR_BPS` (default `100` bps / `1%`, reason `NATR_BELOW_MIN`) and configurable trend controls (`TREND_PENALTY_W`, optional `TREND_HARD_GATE_BPS`). Decision: Phase 1 implementation remains shadow-only (observability path, no dispatch-universe mutation).
