...
- **Pending-place visibility gate (post-PR6):** `_grid_v2_pending_place_cids: dict[str, int]` tracks EXECUTED PLACE CIDs until visible in account snapshot or 2-sync-cycle grace expires. Prevents false-fill cascade on newly placed CIDs. BLOCKED/SKIPPED/NON_RETRYABLE clean registry immediately. Ambiguous FAILED (MAX_RETRIES_EXCEEDED) → quarantine as pending, snapshot resolves. 47 bridge tests (25 classes).
