...
- **Branch-mode one-sided enforcement (PR #402):** Integrity repair enforces one-sided entries in branch mode — opposite-side entries are canceled. SM rejects opposite-side fills as `BRANCH_INCOMPATIBLE` (state.py:428-431); retaining opposite-side entries on exchange creates fill desync risk (engine would `REJECTED_FILL_CLEANED` without lifecycle tracking). Doc-27 section 14 updated. Prior behavior (maintain opposite-side chain) was inconsistent with SM contract.
