...
- **Market-data venue fix (post-PR6):** `--exchange-port futures` switches WS market data to `fstream.binance.com` (was `stream.binance.com` spot). Symbol-vs-venue preflight validation: fail-closed if symbol not in futures exchangeInfo. Fixes: futures-only symbols (e.g. FARTCOINUSDT) now receive bookTicker stream.
