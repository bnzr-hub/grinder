...
- **New model core:** Explicit state machine with separation of concerns. Entry window (bounded, rolling), inventory ledger (lot-level tracking), exit map (paired exits outside window), rolling controller (entry-only mutations). Deterministic transitions, exchange-truth reconciliation, one-sided inventory branches.
