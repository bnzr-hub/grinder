...
- **Launch guard v2:** `run_trading.py` startup for futures+mainnet+armed now runs `preflight → verify → start` (fail-closed). If exchange state is DIRTY, startup exits immediately unless `--pre-cleanup` is enabled (runs `exchange_state cleanup` first, then re-verifies). `--skip-launch-guard` opt-out for testnet/paper/debug. Closes Gate A from doc-36. `exchange_state.py` now exposes `cmd_verify_programmatic()` for programmatic verification (returns structured result, no sys.exit).
