...
- **PR6 delivered:** Live ceremony PASSED (2026-03-18). Price quantization: `GridV2Config.price_tick_size` + `_quantize_price()` in bridge + `GRINDER_GRID_V2_TICK_SIZE` env var. Awaiting-sync gate: `_grid_v2_awaiting_sync` flag skips fill detection until first account sync after fresh start. 3 new precision tests. Ceremony: BTCUSDT, 1 level, 0.002 BTC, 180s, 16167 ticks, 0 errors, FLAT position, cleanup CLEAN. Section 25 (25.1-25.3) added to doc-27 spec.
