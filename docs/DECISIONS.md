...
- **Flat preserve mode (post-PR6):** `GridV2Config.reseed_on_flat` (env `GRINDER_GRID_V2_RESEED_ON_FLAT`, default `true`) controls full-unwind normalization. `true` keeps legacy auto-reseed behavior. `false` disables unconditional full recenter (`RECENTER*`) and keeps rolling continuity (`EXIT_RESTORE`) so missing side levels are replenished on unwind; FLAT integrity still performs cleanup cancels for extra entry/exit drift (`GRID_V2_INTEGRITY_FLAT_PRESERVE`). Optional skew-only reseed is available via `GRINDER_GRID_V2_RESEED_ON_FLAT_ONLY_ON_SKEW=true`: in preserve mode, recenter is re-enabled only when FLAT ladder is BUY/SELL skewed.
...
- **Active ML Phase 3b (doc-36):** ML adjust contract/plumbing wired into `ActiveSelector` via `ShadowSelector` delegation — `ActiveSelectorConfig.ml_enabled/ml_adjust_max_bps` passed through to `to_shadow_config()`. `ml_adjust_provider` accepted in `ActiveSelector.__init__`. All Phase 2 safety invariants preserved (hysteresis, budget, graceful_exit_only, fail-safe). No new env vars — reuses existing `ML_ENABLED` + `ML_ADJUST_MAX_BPS`. **Provider not wired in `run_trading.py`** — ONNX pipeline integration deferred. Runtime `ML_ENABLED=1` in active mode produces `PROVIDER_MISSING` warning + baseline fallback (same behavior as shadow path).
