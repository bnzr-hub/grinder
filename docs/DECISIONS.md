...
- **Symbol Risk Manager v1:** Per-symbol risk escalation layer (Gate 10 in `_process_action`). States: NORMAL → CAPPED → EXIT_ONLY. CAPPED/EXIT_ONLY block INCREASE_RISK for the symbol; reduce-risk/cancel always pass. Grid_v2 internal actions (repair/seed/recenter) bypass the gate. Escalation triggers: notional exceeds `MAX_NOTIONAL_USD`, qty exceeds `MAX_POSITION_QTY`, consecutive losses ≥ `MAX_CONSECUTIVE_LOSSES` (per closed lot P&L, goes directly to EXIT_ONLY). De-escalation: step down one level when conditions safe + cooldown expired + EXIT_ONLY TTL passed. Feature-flagged: `GRINDER_SYMBOL_RISK_ENABLED=0` default. Applies to grid_v2 symbols only by default (`APPLIES_TO_GRID_V2_ONLY=1`).
