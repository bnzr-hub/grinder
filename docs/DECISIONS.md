...
- **Doc-36 Phase 0 contract baseline (docs-only):** `docs/36_MULTI_SYMBOL_ELIGIBILITY_ML_INTEGRATION_V1.md` now freezes initial selector contract artifacts before any runtime wiring: env config contract, deterministic exclusion reason codes, initial selector metrics schema, and hard volatility floor `NATR_14_5m >= 1%` (`100 bps`, reason `NATR_BELOW_MIN`). Decision: Phase 1 implementation remains shadow-only (observability path, no dispatch-universe mutation).
