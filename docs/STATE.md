...
- **Phase C2a (deactivation verification):** [DONE] `evaluate_deactivation()` in `src/grinder/orchestration/deactivation.py` — pure cleanup gate: finalize only when flat + no open orders. 3 block reasons, 5 stages. No live cleanup execution (ADR-129). 14 tests.
