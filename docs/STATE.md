...
- **Ranker V2 scoring engine** (`src/grinder/selector/ranker.py`): [DONE] Seven-component formula: `0.25L + 0.15S + 0.20V + 0.25R + 0.15E - 0.15T - 0.10X`. Rewards choppiness (range_score), penalizes trending (net_return_bps) and toxicity (spread-based). Execution fit rewards symbols under runtime caps. `SelectionFeaturesV2`, `ScoredSymbolV2`, `fetch_selection_features_v2()`, `rank_v2()` — all additive alongside V1. No new REST calls. Not yet wired into AutonomousLoop (follow-on PR). 10 ranker tests + 14 feature tests = 24 total. (ADR-154).
