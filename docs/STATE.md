...
- **Rolling Infinite Grid** (ADR-085):
  ...
  - **V1E (convergence guard + cancel-spin fixes, BUG-3/BUG-4):** Live run8 exposed two operational bugs:
    ...
    - 4 BUG-3 tests (log throttle, reset after sync, pure shift no-relatch, net-new relatch) + 9 BUG-4 tests (populated, selective prune on sync, persists without sync, error path, empty no-log, skip-in-loop, same-tick dedup, different CIDs pass, blacklist survives visible CID, blacklist pruned absent CID).
