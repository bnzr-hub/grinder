...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Post-PR6 hardening:** disappeared-fill processing order is deterministic (`EXIT` before `ENTRY`) to permit same-tick unwind/branch-switch transitions. Rejected disappeared CIDs are cleaned from registry (`GRID_V2_REJECTED_FILL_CLEANED`) to prevent repeat reject loops.
