...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Market-data venue fix:** `--exchange-port futures` now uses `fstream.binance.com` for WS market data (was `stream.binance.com` spot). Symbol-vs-venue preflight validation added (fail-closed). `BINANCE_WS_FUTURES_MAINNET` constant in `binance_ws.py`.
