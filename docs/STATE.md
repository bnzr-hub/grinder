...
- **Phase D1 (universe provider):** [DONE] `filter_candidates()` in `src/grinder/orchestration/universe_provider.py` — venue-level discovery: USDT perpetual, TRADING status, blacklist exclusion. Alphabetical sort. No prefilter/tuning/ranking (ADR-131). 17 tests.
- **Phase D2 (continuous loop):** [NOT STARTED] Full autonomous rotation without `--symbols`.
