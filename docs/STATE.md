...
- **Phase E3 (graceful exit + deactivation):** [IMPLEMENTATION READY] `signal_graceful_exit()` + `deactivate()` in execution_plane. Signal primitive via injectable hook. Real E3 proof requires: graceful-exit signal → no-new-entries evidence → finalize gate → deactivation ceremony → clean exchange. SIGINT shutdown/cleanup proof (C2b pattern) is insufficient for E3 gate. (ADR-136). 21 tests.
