...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Geometry watchdog repair:** Integrity repair detects not only structural mismatches (missing/extra keys) but also geometry mismatches (order price drifts beyond `EPSILON_TICKS × tick_size` from expected). Uses fuzzy matching: orders within tolerance count as "matched" (no false structural cancel/place). Misaligned orders get cancel+place to correct price. Anti-churn: cooldown (`GEOMETRY_COOLDOWN_MS=5000`), budget cap (`GEOMETRY_MAX_ACTIONS_PER_CYCLE=3`). Enabled by default (`GRINDER_GRID_V2_GEOMETRY_REPAIR_ENABLED=1`). 12 tests.
