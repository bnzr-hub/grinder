...
- **Cold-start tuning bootstrap:** [DONE] `_bootstrap_tuning_cache()` in `run_autonomous.py` fetches REST prices at startup and populates TuningCache before first loop cycle. Without this, autonomous loop sees CACHE_MISS for every symbol → tuned=0 → no activation (bootstrap deadlock). Fail-open: price fetch failure leaves symbol un-tuned. (ADR-152).
