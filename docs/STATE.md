...
- **Rolling Infinite Grid** (ADR-085):
  ...
  - **V1E (convergence guard + cancel-spin fixes, BUG-3/BUG-4):** Live run8 exposed two operational bugs:
    ...
    - **BUG-4 follow-up (ADR-090, duplicate cancel dedup):** Ceremonies A3c/A4 exposed three duplicate cancel mechanisms: (1) same-tick — planner `GRID_TRIM` + cycle_layer `TP_SLOT_TAKEOVER` both target same CID, concatenated without dedup; (2) cross-sync — unconditional `_cancel_failed_ids.clear()` wiped blacklist even when CID still visible in snapshot; (3) planner→sync→dispatch stale-action race — planner builds CANCEL from stale snapshot, sync refreshes mid-tick (order gone), dispatch sends stale CANCEL. Fix: per-tick `cancel_dispatched_this_tick` dedup set + selective blacklist prune + dispatch-time stale-action filter (verify `order_id` in current `_last_account_snapshot.open_orders` before dispatching CANCEL).
    - **Logs:** `CANCEL_FAILED_IDS_PRUNED pruned={} surviving={} gen={}` (on sync refresh). `CANCEL_SKIP_DUPLICATE` (same-tick dedup). `CANCEL_SKIP_STALE_ACTION` (stale-action filter). `BlockReason.CANCEL_ALREADY_FAILED` (SKIPPED status, reused for all three paths).
    - 4 BUG-3 tests (log throttle, reset after sync, pure shift no-relatch, net-new relatch) + 12 BUG-4 tests (populated, selective prune on sync, persists without sync, error path, empty no-log, skip-in-loop, same-tick dedup, different CIDs pass, blacklist survives visible CID, blacklist pruned absent CID, stale cancel skipped after sync, legitimate cancel dispatches when order present).
