...
- **Phase E3 (graceful exit + deactivation):** [TOOLING READY] Implementation + ceremony harness. `force_graceful_exit()` added to `LiveEngineV0`. SIGUSR1 handler in `run_trading.py`. `scripts/e3_graceful_exit_ceremony.py` + runbook 41. Bounded live proof pending operator execution. (ADR-136).
