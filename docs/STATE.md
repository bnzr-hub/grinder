...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Symbol Risk Manager v1:** Per-symbol risk controller (`GRINDER_SYMBOL_RISK_ENABLED=0` default). States: NORMAL → CAPPED → EXIT_ONLY. Triggers: notional cap (`MAX_NOTIONAL_USD`), qty cap (`MAX_POSITION_QTY`), consecutive losses (`MAX_CONSECUTIVE_LOSSES`, counted per closed lot: LONG exit<entry or SHORT exit>entry). Gate 10 in `_process_action` blocks INCREASE_RISK in CAPPED/EXIT_ONLY (grid_v2 internal actions bypass). De-escalation: cooldown + TTL + conditions safe. 19 tests.
