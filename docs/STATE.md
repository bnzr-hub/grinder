...
- **Rolling Infinite Grid** (ADR-085):
  ...
  - **V1E (convergence guard + cancel-spin fixes, BUG-3/BUG-4):** Live run8 exposed two operational bugs:
    ...
    - **BUG-4 follow-up (ADR-090, duplicate cancel dedup):** Ceremony A3c exposed two duplicate cancel mechanisms: (1) same-tick — planner `GRID_TRIM` + cycle_layer `TP_SLOT_TAKEOVER` both target same CID, concatenated without dedup; (2) cross-sync — unconditional `_cancel_failed_ids.clear()` wiped blacklist even when CID still visible in snapshot. Fix: per-tick `cancel_dispatched_this_tick` dedup set + selective blacklist prune (set intersection with fresh snapshot `open_orders`).
    - **Logs:** `CANCEL_FAILED_IDS_PRUNED pruned={} surviving={} gen={}` (on sync refresh). `CANCEL_SKIP_DUPLICATE` (same-tick dedup). `BlockReason.CANCEL_ALREADY_FAILED` (SKIPPED status, reused for both paths).
    - 4 BUG-3 tests (log throttle, reset after sync, pure shift no-relatch, net-new relatch) + 10 BUG-4 tests (populated, selective prune on sync, persists without sync, error path, empty no-log, skip-in-loop, same-tick dedup, different CIDs pass, blacklist survives visible CID, blacklist pruned absent CID).
