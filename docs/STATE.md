...
- **Two-Sided Rolling Window Grid** (doc-27):
  - **Status:** PR6 delivered + post-ceremony hardening. Live ceremony PASSED (2026-03-18). 26 shadow tests (13 classes) + 65 bridge tests (31 classes) + 99 adapter tests (16 classes) + 86 SM tests, all passing.
  ...
  - **Post-PR6 hardening:** disappeared-fill processing order is deterministic (`EXIT` before `ENTRY`) to permit same-tick unwind/branch-switch transitions. Rejected disappeared CIDs are cleaned from registry (`GRID_V2_REJECTED_FILL_CLEANED`) to prevent repeat reject loops. Flat normalization now supports preserve mode via `GRINDER_GRID_V2_RESEED_ON_FLAT` (default `true`): when disabled, full unwind to FLAT skips full recenter (no `RECENTER*`) but still applies rolling `EXIT_RESTORE` to replenish missing side levels; FLAT integrity skips reseed/placement and emits cleanup cancels for extra entry/exit drift.
