...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Sync-driven reconciler (shadow, ADR-096):** `src/grinder/grid_v2/sync_reconciler.py` — pure function `reconcile_grid_state()` computes deterministic repair actions from fresh AccountSnapshot + SM state. Called on each successful `_tick_account_sync()` in shadow mode (compute + log, no dispatch). Env: `GRINDER_GRID_V2_SYNC_RECONCILER_ENABLED=0` (default off), `GRINDER_GRID_V2_SYNC_RECONCILER_SHADOW=1`. Output: CANCEL extras first, then PLACE missing, budget-capped. 11 tests.
