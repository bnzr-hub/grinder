...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Doc-36 Phase 0 (docs-only):** `docs/36_MULTI_SYMBOL_ELIGIBILITY_ML_INTEGRATION_V1.md` upgraded from candidate sketch to Phase 0 contract baseline. Frozen artifacts for upcoming implementation PRs: selector env contract, deterministic exclusion reason codes, initial selector metrics schema. Contract now includes hard volatility floor `NATR_14_5m >= 1%` (`100 bps`) with exclusion reason `NATR_BELOW_MIN`. No runtime/dispatch changes yet; Phase 1 remains shadow-only wiring.
