...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Geometry watchdog repair:** Integrity repair detects not only structural mismatches (missing/extra keys) but also geometry mismatches (order price drifts beyond `EPSILON_TICKS × tick_size` from expected). Uses fuzzy matching: orders within tolerance count as "matched" (no false structural cancel/place). Anti-churn: cooldown (`GEOMETRY_COOLDOWN_MS=5000`), budget cap (`GEOMETRY_MAX_ACTIONS_PER_CYCLE=3`). Enabled by default (`GRINDER_GRID_V2_GEOMETRY_REPAIR_ENABLED=1`). Same-tick dedup: fill-path PLACE slots are excluded from repair-path to prevent duplicate orders. Strict geometry mode (`GRINDER_GRID_V2_REPAIR_STRICT_GEOMETRY=0` default): when enabled, structural missing slots are repaired even when distance guard would skip. 20 tests.
