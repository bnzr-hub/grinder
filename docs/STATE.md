...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **ML-assisted scoring Phase 3 (doc-36):** Shadow-only bounded ML adjustment in `ShadowSelector`. Env: `GRINDER_SYMBOL_SELECTOR_ML_ENABLED=1` + `ML_ADJUST_MAX_BPS`. Provider callback interface, fail-open fallback, re-rank after adjust. 3 ML metric families. No dispatch mutation. 9 tests.
