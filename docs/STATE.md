...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Launch guard v2:** `run_trading.py` startup sequence now runs `preflight → (optional cleanup) → verify → start` for futures+mainnet+armed. Fail-closed: if exchange state is DIRTY and `--pre-cleanup` is not enabled, process exits before starting trading loop. `--skip-launch-guard` available for testnet/paper/debug. Closes Gate A from doc-36.
