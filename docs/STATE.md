...
- **Phase E3 (graceful exit + deactivation):** [IMPLEMENTATION READY] `signal_graceful_exit()` in `graceful_exit.py` — signal primitive via injectable hook (ACTIVE → GRACEFUL_EXIT). `deactivate()` in `engine_deactivation.py` — finalize-gated, cleanup-verified (GRACEFUL_EXIT → SHUTTING_DOWN → STOPPED/FAILED). All deps injectable. Real dispatch-gate integration and bounded live proof (activate → graceful exit → deactivate → clean) pending per gate E3→E4 (ADR-136). 21 tests.
...
- **Phase E6 (coordinator):** [IMPLEMENTATION READY] `ExecutionCoordinator` in `src/grinder/execution_plane/coordinator.py` — maps corrective actions to ceremonies with 4-gate chain: enabled → ACK → safety → pause. Default: shadow-only. All ceremonies injectable. (ADR-139). 12 tests. Actual wiring into `AutonomousLoop.run_cycle()` pending. Execution-plane plan completion blocked by E3 live proof gate.
