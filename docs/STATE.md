...
- **Autonomous runtime wiring:** [DONE] `run_autonomous.py` wired to `AutonomousEngineHost`. Coordinator ceremonies bound to real host methods (`host.activate`, `host.request_graceful_exit`, `host.finalize_deactivation`). Shutdown path calls `host.shutdown_all()` in finally block. Shadow/ACK semantics preserved. Engine instances are lightweight markers — real LiveEngineV0 integration deferred to PR 3. 9 wiring tests. (ADR-148).
