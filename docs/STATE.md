...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Suppress Reconciler Burst Churn (ADR-111):** PLACE_ENTRY staging suppressed only when `snapshot.ts < last_fill_ts` (snapshot stale relative to fills). When snapshot catches up, PLACEs allowed normally. No starvation. Log: `GRID_V2_BURST_CHURN_SUPPRESSED`. 8 tests.
