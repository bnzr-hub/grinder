...
- **Phase E3 (graceful exit + deactivation):** [DONE] `signal_graceful_exit()` + `deactivate()` in execution_plane. Bounded live proof captured 2026-04-01 on BTCUSDT @ `4057e98`: preflight CLEAN → 60s trading → SIGINT (graceful shutdown) → cleanup-on-exit → post-verify CLEAN (orders=0, position=FLAT). Signal primitive via injectable hook. (ADR-136). 21 tests.
