...
- **LiveEngineBridge:** [DONE] `src/grinder/runtime/live_engine_bridge.py` — creates real per-symbol engine threads with NoOpExchangePort. Each symbol gets: background thread + async loop + LiveConnectorV0 + LiveEngineV0. `factory()` starts thread, fail-closed if engine not ready in 10s. `stop()` signals shutdown + joins thread. `graceful_exit()` calls real `engine.force_graceful_exit()`. Bridge wired into `run_autonomous.py` via host. Production exchange port integration pending. 11 bridge tests + 6 wiring tests. (ADR-149).
