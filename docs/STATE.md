...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **EventLedger Shadow Mode (ADR-109 Phase 1):** Order-only local event-derived state from ORDER_TRADE_UPDATE. Shadow comparison against snapshot open orders on each sync. Zero behavioral change — observability only. Position tracking deferred to Phase 2. `EVENT_LEDGER_SHADOW_DIVERGENCE` log. 13 tests.
