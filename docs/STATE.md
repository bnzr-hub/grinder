...
- **Rolling Infinite Grid** (ADR-085):
  ...
  - **V1F (anchor/initial placement contract, ADR-088, INV-10):** Live runs (run9-run13) exposed grid asymmetry after external cleanup. Root cause: no re-anchor mechanism when exchange empties while engine is running.
    ...
    - **ANCHOR_RESET success path: PROVEN (white-box)** (2026-04-02, ADR-151). Integration tests T63-T65 prove the full reset sequence: fill → position → flat → 5 conditions → ANCHOR_RESET → new grid same-tick. Tests use white-box state setup (manual `_prev_rolling_orders`/`_inflight_shift` clearing) to isolate the reset path. Fully natural no-intervention live proof deferred to next live ceremony.
