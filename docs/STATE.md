...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Active ML Phase 3b (doc-36):** ML adjust contract/plumbing wired into `ActiveSelector` via `ShadowSelector` delegation. Same bounded model, reuses ML env vars. All Phase 2 safety preserved. **Provider not wired in run_trading** — ONNX integration pending. Runtime `ML_ENABLED=1` produces `PROVIDER_MISSING` warning + baseline fallback. 9 tests.
