...
- **Two-Sided Rolling Window Grid** (doc-27):
  ...
  - **Doc-36 Phase 0 (docs-only):** `docs/36_MULTI_SYMBOL_ELIGIBILITY_ML_INTEGRATION_V1.md` upgraded from candidate sketch to Phase 0 contract baseline. Frozen artifacts for upcoming implementation PRs: selector env contract, deterministic exclusion reason codes, initial selector metrics schema, and configurable threshold contract (including `GRINDER_SYMBOL_SELECTOR_MIN_NATR_BPS`, default `100` bps / `1%`, reason `NATR_BELOW_MIN`, plus trend penalty/hard-gate params). No runtime/dispatch changes yet; Phase 1 remains shadow-only wiring.
