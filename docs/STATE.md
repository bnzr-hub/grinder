...
- **BinanceFuturesPort v0.1** (`src/grinder/execution/binance_futures_port.py`):
  ...
  - **Non-ASCII symbol sanitization (2026-04-13, ADR-182):**
    ...
    - `cid_symbol_matches(symbol, parsed_token)` is the authoritative reverse-lookup comparator: ASCII symbols require **exact equality** (prevents false positives like `"BTCUSDT".startswith("BTC")`); non-ASCII symbols use `startswith` (CID stores truncated hash)
...
- **Rolling Infinite Grid** (ADR-085):
  ...
  - **V1E (convergence guard + cancel-spin fixes, BUG-3/BUG-4):** Live run8 exposed two operational bugs:
    ...
    - **BUG-4 (CANCEL_2011 spin):** Stale snapshot shows already-cancelled order → planner generates CANCEL → Binance returns -2011 → repeats every tick until sync refresh. Fix: `_cancel_failed_ids` blacklist. Failed CANCELs recorded; blacklisted order_ids skipped in action loop. Selectively pruned on AccountSync refresh (CIDs absent from fresh snapshot removed; CIDs still visible kept — ADR-090 follow-up).
    - **BUG-4 follow-up (ADR-090, duplicate cancel dedup):** Ceremonies A3c/A4/A5 exposed four duplicate cancel mechanisms: (1) same-tick — planner `GRID_TRIM` + cycle_layer `TP_SLOT_TAKEOVER` both target same CID, concatenated without dedup; (2) cross-sync — unconditional `_cancel_failed_ids.clear()` wiped blacklist even when CID still visible in snapshot; (3) planner→sync→dispatch stale-action race — planner builds CANCEL from stale snapshot, sync refreshes mid-tick (order gone), dispatch sends stale CANCEL; (4) cross-tick successful-cancel re-dispatch — CANCEL succeeds on tick N, planner re-generates on tick N+1 (snapshot not refreshed), per-tick dedup set was reset. Fix: per-sync-cycle `_cancel_dispatched_pending_sync` dedup set (covers mechanisms 1+4) + selective blacklist prune (mechanism 2) + dispatch-time stale-action filter (mechanism 3).
    - **Logs:** `CANCEL_FAILED_IDS_PRUNED pruned={} surviving={} gen={}` (on sync refresh). `CANCEL_SKIP_DUPLICATE` (per-sync-cycle dedup). `CANCEL_SKIP_STALE_ACTION` (stale-action filter). `BlockReason.CANCEL_ALREADY_FAILED` (SKIPPED status, reused for all four paths).
    - 4 BUG-3 tests (log throttle, reset after sync, pure shift no-relatch, net-new relatch) + 14 BUG-4 tests (populated, selective prune on sync, persists without sync, error path, empty no-log, skip-in-loop, same-tick dedup, different CIDs pass, blacklist survives visible CID, blacklist pruned absent CID, stale cancel skipped after sync, legitimate cancel dispatches when order present, cross-tick cancel dedup, dedup set cleared on sync).
    - **Verdict:** FIXED and LIVE-VERIFIED. Ceremony A6 (2026-03-16, `main @ 6c1850e`): `CANCEL_2011_DIAG=0`, 4650 ticks / 90s, cleanup/verify CLEAN.
  - **V1F (anchor/initial placement contract, ADR-088, INV-10):** Live runs (run9-run13) exposed grid asymmetry after external cleanup. Root cause: no re-anchor mechanism when exchange empties while engine is running.
    ...
    - **Minimum live acceptance: PASSED** (2026-03-15). Proven: ANCHOR_INIT, order placement (2x POST 200 OK), 0 errors, clean shutdown (300s, 3073 ticks), post-run status=CLEAN.
    - **Fill path acceptance: PASSED** (2026-03-16, ceremony A3c). Proven: `INFLIGHT_FILL_DETECTED` (instant BUY fill), `ROLLING_FILL_OFFSET` (both inflight + disappearance paths), `fill→TP` (`TP_SLOT_TAKEOVER` + reduceOnly SELL), no same-price BUY churn, `PLACEMENT_CAPPED` not observed, cleanup/verify CLEAN. Duplicate cancel `-2011` addressed in ADR-090 follow-up (ceremony A6).
    - **ANCHOR_RESET success path: PROVEN (fixture-natural)** (2026-04-02, ADR-151). T63-T65 prove the full reset sequence with zero internal state mutation: init → orders confirmed → exchange empties + flat → ANCHOR_RESET fires → new grid from new anchor same-tick. Only external signals injected (account snapshots, sync gen). Engine manages `_prev_rolling_orders`, `_inflight_shift`, pending cancels entirely on its own.
  ...
  - **V1G (inflight fill detection, ADR-090, INV-11):** Live ceremony A2 (2026-03-15) exposed P0 safety bug: 13 BUY orders in 60s, 11 filled instantly, $1,571 accumulated with zero fills detected. Root cause: disappearance heuristic requires order to appear in at least one REST snapshot — orders filling within one sync interval (~5s) are structurally invisible.
    ...
    - **ADR-090 live verification: PASSED** (2026-03-16, ceremony A3c on `main @ 86f9810`). `INFLIGHT_FILL_DETECTED` observed on real instant BUY fill (CID `grinder_d_BTCUSDT_1_1773675549_1`, dispatch_gen=1→current_gen=2). Two fill paths proven live: inflight CID reconciliation (fill #1) + disappearance heuristic (fill #2). No repeated same-price BUY churn (2 BUYs at different prices). `PLACEMENT_CAPPED` not observed. `fill→TP` path observed (`TP_SLOT_TAKEOVER` + reduceOnly SELL). Known follow-up: 4× duplicate cancel `-2011` (idempotent, convergence loop re-issues cancel before REST refresh — P1 operational gap, not safety issue).
...
- **Two-Sided Rolling Window Grid** (doc-27):
  - **Status:** PR6 delivered + post-ceremony hardening. Live ceremony PASSED (2026-03-18). 26 shadow tests (13 classes) + 70 bridge tests (31 classes) + 99 adapter tests (16 classes) + 86 SM tests, all passing.
  ...
  - **PR5 scope:** Shadow verification (`GridV2ShadowRunner`). Isolated bridge runs grid_v2 in parallel on same snapshots without dispatch. `GRINDER_GRID_V2_SHADOW` env var. Mutual exclusion with primary. Fail-open error handling. Divergence detection (count/type mismatch). Section 24 (24.1-24.5) added to doc-27.
  - **PR6 scope:** Live ceremony (2026-03-18). Price quantization (`price_tick_size` in GridV2Config, `GRINDER_GRID_V2_TICK_SIZE`). Awaiting-sync gate (`_grid_v2_awaiting_sync` flag skips fill detection until account sync). BTCUSDT 1-level 0.002 BTC, 180s, 16167 ticks, 0 errors, FLAT, cleanup CLEAN. Section 25 (25.1-25.3) added to doc-27.
  - **Post-PR6 hardening:** disappeared-fill processing order is deterministic (`EXIT` before `ENTRY`) to permit same-tick unwind/branch-switch transitions. Rejected disappeared CIDs are cleaned from registry (`GRID_V2_REJECTED_FILL_CLEANED`) to prevent repeat reject loops. Flat normalization supports three modes: `GRINDER_GRID_V2_RESEED_ON_FLAT=true` (always recenter), preserve mode (`GRINDER_GRID_V2_RESEED_ON_FLAT=false`), and skew-only reseed (`GRINDER_GRID_V2_RESEED_ON_FLAT=false` + `GRINDER_GRID_V2_RESEED_ON_FLAT_ONLY_ON_SKEW=true`) where FLAT recenter occurs only on BUY/SELL ladder skew; otherwise preserve path keeps rolling continuity and cleanup-cancel behavior.
  - **Observability hardening (post-PR6):** user-data unknown events are now counted in connector metrics with bounded `event_type` labels (`known` + `other`) and surfaced via `grinder_user_data_unknown_events_total` for alerting/triage noise control. No trading behavior changes.
  - **Observability hardening (post-PR427):** grid_v2 warning-path metrics added to live engine: `grinder_live_grid_v2_integrity_mismatch_pending_total{sym}` and `grinder_live_grid_v2_rejected_fill_cleaned_total{sym,source,reason}`. Alert thresholds added: `GridV2IntegrityMismatchPendingBurst` (>5/15m) and `GridV2RejectedFillCleanedBurst` (>3/15m). Unknown user-data warning logs now include explicit `event_type` for faster triage.
  - **Launch guard v2 + crash-safe recovery:** `run_trading.py` startup sequence runs `preflight → recovery policy → (optional cleanup) → verify → start`. Recovery policy: non-flat position → cleanup skipped (startup proceeds for grid_v2 reconstruction); FLAT + dirty → requires 2 stable snapshots before cleanup (guards against pending fills). New statuses: `recovery_non_flat_skip_cleanup`, `recovery_snapshot_unstable`. Pre-cleanup and cleanup-on-exit default ON.
  - **F2 protective recovery:** When restart finds non-flat position but no exit orders on exchange (F2 condition), adapter synthesizes a protective lot + exit instead of fail-closed ValueError. Engine emits reduce-only PLACE_EXIT. No increase-risk entries until protection confirmed. Covers both cases: entries exist but no exits, and no grid_v2 orders at all. Uses signed position qty (`PositionSnap.signed_qty`) for correct exit side: LONG→SELL, SHORT→BUY.
  - **Grid_v2 startup recovery modes:** `GRINDER_GRID_V2_RECOVERY_MODE` controls behavior on reconstruction failure. `restore_then_cleanup_reseed` (default): cleanup exchange state → verify CLEAN → startup_fresh with full entry window. `restore_then_block`: fail-closed, no dispatch. Cleanup fallback requires `ALLOW_MAINNET_TRADE=1`. If cleanup/verify fails → fail-closed.
  - **Net-off transition (Variant B):** `GRINDER_GRID_V2_NETOFF_ENABLED=1` (default off). Opposite-side entry fill in branch mode closes closest lot instead of `BRANCH_INCOMPATIBLE` reject. LONG+SELL→close LONG lot, SHORT+BUY→close SHORT lot. Paired exit marked CANCELED and cancel action emitted. Mixed inventory still forbidden. Rolling window update applied normally.
  - **Integrity repair anti-churn guards (PR-B):** Three guards on repair PLACE actions: (1) pending-slot dedup — skip if `(side, price)` already in `pending_place_cids`; (2) distance guard — skip if `abs(price - mid) / mid > max_distance * step_pct` (env `GRINDER_GRID_V2_REPAIR_MAX_DISTANCE_STEPS`, default 5); (3) budget cap — max N PLACE actions per repair cycle (env `GRINDER_GRID_V2_REPAIR_MAX_ACTIONS_PER_CYCLE`, default 5). Prevents repair churn loops, dead-far entries, and action floods.
  - **Symbol Risk Manager v1:** Per-symbol risk controller (`GRINDER_SYMBOL_RISK_ENABLED=0` default). States: NORMAL → CAPPED → EXIT_ONLY. Triggers: notional cap (`MAX_NOTIONAL_USD`), qty cap (`MAX_POSITION_QTY`), consecutive losses (`MAX_CONSECUTIVE_LOSSES`, counted per closed lot: LONG exit<entry or SHORT exit>entry). Gate 10 in `_process_action` blocks INCREASE_RISK in CAPPED/EXIT_ONLY (only safety-critical grid_v2 actions bypass: cancel, reduce-only, integrity cancel). De-escalation: cooldown + TTL + conditions safe. 21 tests.
  - **Symbol Unload Controller v1:** Staged reduce-only position unload while in EXIT_ONLY state (`GRINDER_SYMBOL_UNLOAD_ENABLED=0` default). Activated automatically when risk manager escalates to EXIT_ONLY. Per-step: compute qty (step_pct × abs_position, clamped to min_step_qty), place reduce-only order (LONG→SELL, SHORT→BUY). Cooldown between steps (`STEP_COOLDOWN_S=20`), hourly cap (`MAX_STEPS_PER_HOUR=60`), reprice TTL (`ORDER_TTL_S=60`, `MAX_REPRICES_PER_STEP=3`). On flat → UNLOAD_COMPLETE. On 3 consecutive failures → PAUSED (stays EXIT_ONLY). 17 tests.
  - **Adaptive Step Controller v1:** Volatility-aware grid step (`GRINDER_GRID_V2_ADAPTIVE_STEP_ENABLED=0` default). Computes effective step from NATR: `step = base × (vol_bps / vol_ref_bps)`, clamped to [min, max]. Hysteresis prevents tiny oscillations. Cooldown prevents frequent updates. Fail-open: missing vol → `freeze_last` (default) or `fallback_base`. v1 applies effective step only on bridge recreation (reseed/restart), not mid-flight recenter. 17 tests.
  - **Geometry watchdog repair (role-aware):** Integrity repair is role-separated: ENTRY geometry uses fuzzy tick-tolerance matching against SM entry_window; EXIT integrity checks lot-ledger expected CIDs against exchange. ENTRY cancel only targets ENTRY CIDs, EXIT cleanup only targets EXIT CIDs. Anti-churn: cooldown, budget cap, same-tick dedup, strict geometry mode. In branch mode, missing exits are logged (`GRID_V2_EXIT_INTEGRITY_MISMATCH`) for observability. Exit orphan accounting is `OPEN`-only (historical `FILLED/CANCELED` exits are excluded from active orphan checks). **Geometry mismatch detection enabled:** same-side extra orders outside epsilon are paired with nearest missing and reported as geometry mismatches (cancel+replace). **Bounded convergence deferral:** integrity repair deferred at most 6 ticks during convergence; then escalates with forced stale-pending resolution. **Stale pending lifecycle:** on escalation, pending cancels older than 15s and pending places older than 4 sync generations are force-resolved (cancel ack if gone from exchange, drop from blocker set if still present). **Escalation log throttle:** logs on first escalation + every 10th defer (not per-tick). **Streak preservation:** mismatch streak is not reset during convergence deferral nor on escalation. 34 tests.
  - **Sync-driven reconciler (ADR-096):** `src/grinder/grid_v2/sync_reconciler.py` — pure function `reconcile_grid_state()` computes deterministic repair actions from fresh AccountSnapshot + SM state. Two modes: **shadow** (compute + log, no dispatch) and **primary** (dispatch on sync cycle, tick watchdog demoted). Env: `GRINDER_GRID_V2_SYNC_RECONCILER_ENABLED=0` (default off), `GRINDER_GRID_V2_SYNC_RECONCILER_SHADOW=1` (shadow), `GRINDER_GRID_V2_SYNC_RECONCILER_PRIMARY=0` (primary). PRIMARY+SHADOW mutually exclusive (fail-closed). In primary mode, reconciler actions staged on sync and drained on next process_snapshot tick. Fill/cancel-ack detection still runs every tick. 17 tests.
  - **Reduce-only budget guard (ADR-099):** Gate 0 in `_process_action()`. Aggregate reduce-only qty check: `existing_ro + batch + new <= effective_position`. Position = exchange snapshot SSOT + provable current-tick lot additions. Within-tick batch accumulator prevents over-dispatch. Stale registry pre-pass before reconciler. 8 tests.
  - **Exit step-spacing guard (ADR-098 update):** In `_execute_entry()`, new exits must maintain min spacing of `step_price` from all open exits. Shifts by full `step_price` units. Fail-closed on search exhaustion (PLACE_EXIT skipped, lot created, exit deferred to reconciler). 5 tests.
  - **Live Health Gate (ADR-100):** `src/grinder/live/health_gate.py` — evaluates truth-source health per tick. Modes: HEALTHY, DEGRADED_SYNC, DEGRADED_WS, STALE_TRUTH, PAUSED_UNSAFE. Write policy: STALE_TRUTH blocks normal PLACE (allows cancel + reduce-only). PAUSED_UNSAFE blocks all writes except cancel. Transitions logged with reason codes. DNS storm detection included. 19 tests.
  - **Live Preflight Gate (ADR-101):** `src/grinder/live/preflight.py` — pre-start checks for armed mainnet runs. Checks: DNS resolution, exchange time sync (fail at 1500ms drift), WS bootstrap (TCP connect probe), account sync read (real syncer wired), symbol metadata (validates target symbols against loaded constraints), config consistency. Fail-closed: hard failure aborts with exit code 2 before trading loop. Read-only (no writes). 26 tests.
  - **Risk-Saturated Mode (ADR-102):** Legal target projection for risk-constrained symbols. Capacity = `min(symbol, gross, net)` headroom across all active Gate 5.5 caps. Only `SYMBOL_CAP_EXCEEDED` with entries actually missing on exchange counts toward saturation counter (passive zero-headroom with satisfied ladder, stale/portfolio/DD blocks don't trigger saturation). Saturation flag clears on reason change. Proactive recovery on sync. Env: `GRINDER_RISK_SATURATION_THRESHOLD`. Logs: `GRID_V2_RISK_SATURATED_ENTER/EXIT`. 21 tests.
  - **Effective Desired State Projection (ADR-103):** Three-layer model: theoretical (SM wants), effective (legal after risk), actual (exchange). Reconciler diffs actual vs effective. `ProjectionMode` enum: `UNCONSTRAINED`, `RISK_CONSTRAINED_PARTIAL`, `RISK_CONSTRAINED_ZERO`. `ReconcileResult` now has `theoretical_desired_entry_count`, `desired_entry_count` (effective), `projection_mode`, `legal_entry_capacity`. Partial projection keeps N closest entries to reference. Pipeline: theoretical → gap supplement → projection → diff actual. Gap detection runs before projection so effective never exceeds capacity. Key invariant: actual matches effective but not theoretical → zero actions. Sync log includes `theoretical_entries`, `effective_entries`, `projection`, `capacity`. 19 tests.
  - **Reduce-Only Budget Guard v2 (ADR-104):** Full reservation model: `open_remaining + reserved + new_qty <= position`. Direction-aware: SELL exit → LONG closeable, BUY exit → SHORT closeable. Partial fills use `qty - filled_qty`. Sync-time repair detects over-budget topology, cancels smallest first. -2022 reject flags `(symbol, side)`, blocks only affected direction until repair converges. Convergence only declared when all cancels succeed; failed cancels keep flag set. New module `src/grinder/live/reduce_only_budget.py`. 31 tests.
  - **Observability + Reason Codes (ADR-106):** Stable reason codes for all major suppressed/degraded/no-op paths. 5 reason families (25 total codes): NoActionReason(6), EntrySuppressionReason(5), ExitSuppressionReason(4), HealthBlockReason(4), RepairStatusReason(6). `GRID_V2_NO_ACTION` and `GRID_V2_ENTRY_SUPPRESSED` logs with structured reasons. Healthy no-op throttled. `classify_no_action_reason()` pure classifier. New module `src/grinder/live/reason_codes.py`. 17 tests (14 taxonomy + 3 engine-path).
  - **Exit Topology Repair (ADR-105):** Deterministic convergence of exchange exit set to desired legal topology. Three-layer model: desired (SM + budget), actual (exchange), repair diff. Cancels extras, places missing (if legal), defers unregistered. Budget-constrained by closeable position. Runs each sync after ADR-104 budget repair. Triggers: `SYNC_DRIFT`, `REJECT_RECOVERY` (emitted); `BUDGET_OVERRUN`, `PARTIAL_FILL_RECOMPUTE` (defined for future use). New module `src/grinder/grid_v2/exit_repair.py`. 16 tests.
  - **Quantize Exit-Repair Placements (ADR-114):** Repair exit prices quantized to tick size before dispatch. Eliminates -4014 rejects. 3 tests.
  - **Exit Topology Repair Re-Registration (ADR-113):** DEFERRED exits (registry lost after -2022) are now re-registered and placed instead of infinite DEFERRED loop. Log: `GRID_V2_EXIT_TOPOLOGY_REPAIR_REREGISTERED`. 8 tests.
  - **Suppress Stale Pre-Fill Placements (ADR-112):** Drop reconciler PLACE_ENTRY when SM mode changed since staging. Prevents stale FLAT-view placements from polluting post-fill topology. Log: `GRID_V2_STALE_MODE_PLACE_DROPPED`. 4 tests.
  - **Suppress Reconciler Burst Churn (ADR-111):** One-shot PLACE_ENTRY suppression for one cycle after fills, then cleared. No restore starvation. Log: `GRID_V2_BURST_CHURN_SUPPRESSED`. 10 tests.
  - **Suppress Replenish When Inventory Full (ADR-110):** SM exit-fill replenish path checks `len(new_open) < max_inventory_levels` before placing EXIT_RESTORE entries. Prevents one source of inadmissible replenish when inventory is already at/above max. Broader reconciler-level burst churn is a separate issue. 2 tests.
  - **EventLedger Shadow Mode (ADR-109 Phase 1 + PR-C):** Order-only local event-derived state from ORDER_TRADE_UPDATE. Bootstrap hydration from first AccountSnapshot eliminates false startup divergence. Shadow comparison on each sync. Zero behavioral change — observability only. `EVENT_LEDGER_BOOTSTRAP_HYDRATED` + `EVENT_LEDGER_SHADOW_DIVERGENCE` logs. 20 tests.
  - **Bounded Sync TS Regression Tolerance (ADR-108):** Tolerate Binance cached-snapshot timestamp regression after 3 consecutive occurrences (< 10s delta). Prevents reconciler starvation from frozen `last_ts`. Large regressions still fail-closed. 7 tests.
  - **Final Integration + Acceptance (ADR-107):** Cross-feature adversarial integration matrix proving all 7 production subsystems compose correctly. 15 integration tests in `tests/integration/test_live_acceptance_matrix.py` (10 cross-feature + 5 readiness command with fake syncer/port). Launch-readiness runbook `docs/runbooks/38_GRID_V2_LAUNCH_READINESS.md` with go/no-go checklist, blocker events, verification success criteria.
  - **Shadow selector Phase 1 (doc-36):** `ShadowSelector` in `src/grinder/selection/shadow_selector.py` runs Top-K v1 scoring on accumulated FeatureSnapshots with timestamp-based cooldown. Post-dispatch engine hook, fail-open. Env: `GRINDER_SYMBOL_SELECTOR_SHADOW=1`. Emits 5 metric families (`grinder_selector_*`), deterministic reason codes, hypothetical churn. No dispatch mutation. 17 tests.
  - **Active selector Phase 2 (doc-36):** `ActiveSelector` in `src/grinder/selection/active_selector.py` extends Phase 1 with controlled activation on operator universe only (no auto-scan). Env: `GRINDER_SYMBOL_SELECTOR_ENABLED=1`. Hysteresis (`MIN_HOLD_CYCLES`, `ENTER/EXIT_THRESHOLD_BPS`), bounded transitions (`MAX_CHANGES_PER_CYCLE`), graceful_exit_only for grid_v2 non-flat symbols. Fail-safe: on error, retains previous stable active set. 14 tests.
  - **ML-assisted scoring Phase 3 (doc-36):** Shadow-only bounded ML adjustment in `ShadowSelector`. Env: `GRINDER_SYMBOL_SELECTOR_ML_ENABLED=1` + `ML_ADJUST_MAX_BPS`. Provider callback interface, fail-open fallback, re-rank after adjust. 3 ML metric families. No dispatch mutation. Provider-missing observability: explicit `SELECTOR_ML_PROVIDER_MISSING` warning at runtime + startup hint. 10 tests.
  - **Active ML Phase 3b (doc-36):** ML adjust wired into `ActiveSelector` via `ShadowSelector` delegation. ONNX provider wired in `run_trading` via `GRINDER_ML_REGIME_MODEL_DIR` env. Regime mapping: HIGH=+base_bps, MID=0, LOW=-base_bps. Fail-open: model load error → baseline fallback. All Phase 2 safety preserved. 9+7 tests.
  - **Market-data venue fix:** `--exchange-port futures` now uses `fstream.binance.com` for WS market data (was `stream.binance.com` spot). Wired end-to-end: `build_connector` → `LiveConnectorConfig.ws_url` → `BinanceWsConfig.ws_url_override` → `BinanceWsConnector.connect()`. Symbol-vs-venue preflight validation: fail-closed if constraints unavailable or symbol missing. Tested: effective URL verified via `_ws_config.ws_url` assertions.
...
- **Phase C2a (deactivation finalize gate):** [DONE] `can_finalize_deactivation()` in `src/grinder/orchestration/deactivation.py` — pure finalize gate: returns can_finalize=True only when flat + no open orders. 3 block reasons. Stage progression owned by RotationController, not this module. No live cleanup execution (ADR-129). 13 tests.
...
- **Phase E3 (graceful exit + deactivation):** [DONE] Bounded live proof captured 2026-04-01 on BTCUSDT @ `29af581`: pre_activity=1, graceful_exit_seen=True, post_entries=0, final verify CLEAN (orders=0, position=FLAT). `force_graceful_exit()` + SIGUSR1 handler + `e3_graceful_exit_ceremony.py` + runbook 41. (ADR-136).
...
- **Legal orders:** [DONE] Tuning solver worst-case deep price (#547), duplicate price prevention in LiveGridPlannerV1 (#548), grid_v2 order size 3-level precedence env > CLI > default (#549).
- **Reseed on skewed FLAT return:** [DONE] State machine exit fill handler now checks `reseed_on_flat_only_on_skew` (was declared but never wired). On return to FLAT from one-sided branch with empty opposite ladder, symmetric entry window is rebuilt. Prevents dead state (e.g. 5 SELL / 0 BUY after LONG round-trip). (ADR-142).
- **Startup contract summary:** [DONE] `src/grinder/live/startup_contract.py` — consolidated operator-facing startup block. Separates LIVE_PREFLIGHT vs LAUNCH_GUARD gates. Prints resolved runtime mode (grid_v2, rolling, planner, account_sync, armed, mainnet, exchange_port). Detects unsafe partial-mode combos (rolling without planner/sync, grid_v2 without symbol/sync, planner without sync, armed+mainnet+noop). Blocks on FAIL gates or BLOCK-severity warnings. (ADR-143).
- **Seed validation and config resolution:** [DONE] Startup contract extended with SEED_READINESS gate (validates grid_v2 symbol, order size, tick size, account sync before seed path). `--preflight-clock-drift-ms` CLI flag for explicit threshold override. RESOLVED CONFIG section in summary prints effective preflight thresholds and grid_v2 sizing (source=env/cli/default). 39 tests. (ADR-144).
- **Run identity:** [DONE] `src/grinder/live/run_identity.py` — stable per-run identity (`YYYYMMDD-HHMMSS-<4hex>`). Printed in structured startup/shutdown banners with run_id, pid, mode, symbols, exchange_port, exit_code, stop_reason. Replaces ad-hoc banners with deterministic formatted blocks. 10 tests. (ADR-145).
- **Structured events and metadata SSOT:** [DONE] `src/grinder/observability/structured_events.py` — typed event helpers for operator triage. `ConstraintSource` enum + `ResolvedConstraint` for per-symbol metadata with truthful source labeling (cache/exchange/stale_cache) at startup. `format_safety_summary()` emitted in coordinator safety gate. `SafetyBlockEvent` and `format_execution_gate_result()` available as formatters for future per-rule emission. 16 tests. (ADR-146).

### Autonomous engine host
- **Engine host skeleton:** [DONE] `src/grinder/runtime/autonomous_host.py` — `AutonomousEngineHost` owns `symbol → engine_ref` map. Real lifecycle: `activate()` (factory → registry ACTIVE), `request_graceful_exit()` (reaches live engine), `finalize_deactivation()` (stop → cleanup → STOPPED), `shutdown_all()` (deterministic, partial-failure report). Injectable factory/stop/cleanup/graceful-exit callables. Registry/runtime coherence tested. 16 tests. (ADR-147).
- **Autonomous runtime wiring:** [DONE] `run_autonomous.py` wired to `AutonomousEngineHost`. Coordinator ceremonies bound to real host methods. Shutdown path calls `host.shutdown_all()` in finally block. Shadow/ACK semantics preserved. (ADR-148).
- **LiveEngineBridge:** [DONE] `src/grinder/runtime/live_engine_bridge.py` — creates real per-symbol engine threads. Each symbol gets: background thread + async loop + LiveConnectorV0 + LiveEngineV0. `factory()` starts thread, fail-closed if engine not ready in 10s. `stop()` signals shutdown + joins thread. `graceful_exit()` calls real `engine.force_graceful_exit()`. (ADR-149).
- **Bridge port wiring:** [DONE] `BridgeConfig.exchange_port` selects NoOp (default) or BinanceFuturesPort. CLI flags `--exchange-port`, `--mainnet`, `--armed`, `--max-notional-per-order`, `--max-orders-per-run`. Futures requires API keys (fail-closed). Startup banner prints effective port mode. Safe defaults: `noop + testnet + unarmed`. 8 port tests. (ADR-150).
- **Cold-start tuning bootstrap:** [DONE] `_bootstrap_tuning_cache()` in `run_autonomous.py` fetches REST prices at startup and populates TuningCache before first loop cycle. Without this, autonomous loop sees CACHE_MISS for every symbol → tuned=0 → no activation (bootstrap deadlock). Solver config wired from bridge ladder geometry (same SSOT). Only real exchange constraints used — missing constraints skip symbol. (ADR-152).
- **Shutdown bypass for no-position engines:** [DONE] `shutdown_all()` forces registry through GRACEFUL_EXIT when `force_graceful_exit()` returns False for benign reasons (no position / read-only). Operator-level override — not a general bypass. Real graceful-exit failures from `request_graceful_exit()` outside shutdown context still fail closed. 19 host tests. Futures WS URL fix: bridge uses correct testnet/mainnet futures WS endpoints.

### Autonomous symbol selection (V2)
- **Selector V1** (`src/grinder/selector/`): [DONE] `prefilter_v1` + `rank_v1` + `fetch_selection_features`. Hard prefilter gates: TUNED, TRADING, blacklist, volume floor, NATR floor, runtime-cap legality. Composite ranker: `0.45 * volume + 0.25 * spread + 0.30 * volatility` with log-scale volume and min-max normalization. Live-validated in autonomous loop. (PR #582).
- **Selector V2 volume metric** (`src/grinder/selector/feature_provider.py`): [DONE] Rolling 12×5m quote volume replaces single 1h candle. `quote_volume_last_12x5m` = sum of last 12 closed 5m kline `quoteAssetVolume`. Hard floor raised from $50k to $2M (`DEFAULT_VOLUME_LAST_12X5M_MIN`). Single unified 5m kline fetch (`limit=18`) serves both volume and NATR(14), eliminating the separate 1h call. Fail-open per symbol. 18 feature_provider tests + 21 selector tests = 39 total. (ADR-153).
- **Ranker V2 scoring engine** (`src/grinder/selector/ranker.py`): [DONE] Seven-component formula: `0.25L + 0.15S + 0.20V + 0.25R + 0.15E - 0.15T - 0.10X`. Rewards choppiness (range_score), penalizes trending (net_return_bps) and toxicity (spread-based). Execution fit rewards symbols under runtime caps. `SelectionFeaturesV2`, `ScoredSymbolV2`, `fetch_selection_features_v2()`, `rank_v2()` — all additive alongside V1. No new REST calls. 10 ranker tests + 14 feature tests = 24 total. (ADR-154).
- **Ranker V2 live wiring** (`scripts/run_autonomous.py`): [DONE] `_build_v2_selector()` replaces `_build_v1_selector()` in `build_runtime()`. `rank_v2` is now the live ranker with fail-open fallback to `rank_v1` if V2 features unavailable. `prefilter_v1` unchanged (uses separate V1 feature fetch). `tuning_order_sizes` and `max_notional_per_order` forwarded to V2 fetch. `SELECTOR_TOP_V2` replaces `SELECTOR_TOP` on ranker path. 6 wiring tests. (ADR-154 wiring complete).
